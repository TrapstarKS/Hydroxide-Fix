...
local remoteLogsMenu =
	ContextMenu.new({ scriptContext, callingScriptContext, spyClosureContext, repeatCallContext, viewAsHexContext })
...
local function createArg(instance, index, value)
	...
	if oh.Constants.Types[valueType] then
	...
	if oh.Constants.Syntax[valueType] then
		arg.Label.TextColor3 = oh.Constants.Syntax[valueType]
	else
		arg.Label.TextColor3 = Color3.new(1, 1, 1)
	end
...
function ArgsLog.new(log, callInfo)
	...
	button:SetRightCallback(function()
		...
		selected.isclientEvent = callInfo.isclientEvent
...
scriptContext:SetCallback(function()
	...
	if selected.isclientEvent then
		...
		script = script .. "\n" .. "for _, v in pairs(getconnections(" .. remotePath .. ".OnClientEvent)) do\n"
		script = script .. "\tv:Fire(" .. args:sub(1, -3) .. ")\n"
		script = script .. "end"
		script = script .. "\n\n\n -- (Original Script) --\n--" .. remotePath .. ":" .. method .. "(" .. args:sub(1, -3) .. ")"
		setClipboard(script)
	elseif #selected.args == 0 then
...
repeatCallContext:SetCallback(function()
	...
	if selected.isclientEvent then
		for _, v in pairs(getconnections(remoteInstance.OnClientEvent)) do
			v:Fire(unpack(selected.args))
		end
	else
...
Methods.ConnectEvent(function(remoteInstance, callInfo)
	if not removed[remoteInstance] then
		local remote = CurrentsOnClientEvent[remoteInstance] or currentRemotes[remoteInstance]
